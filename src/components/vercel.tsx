      ...
      <Card className="w-full max-w-sm gap-4">
        ...
        <CardContent>
          <ItemGroup className="gap-0">
            {items.map((item) => (
              <Item key={item.name} size="xs" className="px-0 group-hover/item-group:bg-transparent" render={<a href="#"><ItemMedia variant="icon" className="text-primary">
                  <CircularGauge percentage={item.percentage} />
                </ItemMedia><ItemContent className="inline-block truncate">
  ...
  return (
    <Example title="Deployment Filter" containerClassName="col-span-full">
      <div className="flex w-full flex-wrap items-center gap-2 *:w-full lg:*:w-auto">
        <Popover>
          <PopoverTrigger render={<Button variant="outline" className="justify-start"><CalendarIcon data-icon="inline-start" className="text-muted-foreground" />{dateRange?.from ? (
              dateRange.to ? (
                <>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger render={<Button variant="outline" className="justify-between">{selectedEnvironment}<ChevronDownIcon data-icon="inline-end" className="text-muted-foreground" /></Button>} />
          <DropdownMenuContent className="w-56" align="end">
            {environments.map((environment) => (
              ...
              >
                {environment}
                <CheckIcon className="ml-auto opacity-0 group-data-[active=true]/dropdown-menu-item:opacity-100" />
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
        <DropdownMenu>
          <DropdownMenuTrigger render={<Button variant="outline" className="justify-between"><div className="flex items-center -space-x-0.5">
              {statuses.map((status) => (
                <div
                  key={status.name}
                  style={
            ...
            </div>Status {selectedStatuses.size}/{statuses.length}<ChevronDownIcon data-icon="inline-end" className="text-muted-foreground ml-auto" /></Button>} />
          <DropdownMenuContent className="w-56" align="end">
            {statuses.map((status) => {
        ...
        {billingItems.map((item, index) => (
          <React.Fragment key={item.month}>
            <Item className="grid grid-cols-[1fr_auto] lg:grid-cols-[2fr_1fr_1fr_auto]">
              ...
              <ItemActions>
                <DropdownMenu>
                  <DropdownMenuTrigger render={<Button variant="ghost" size="icon"><MoreHorizontalIcon
...
function ActivateAgentDialog() {
  return (
    <Example title="Activate Agent" className="items-center justify-center">
      <Dialog>
        <DialogTrigger render={<Button variant="outline">Activate Agent</Button>} />
